 ...
 * Description:播放器
 */
public class JVPlayer extends FrameLayout
        implements IJVPlayer,
    ...
    private JVTextureView mTextureView;
    private JVPlayerController mController;
    ...
    public JVPlayer(Context context) {
        this(context, null);
    }

    public JVPlayer(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        init();
    }
    ...
    public void setController(JVPlayerController controller) {
    ...
    public void setSpeed(float speed) {
        if (mMediaPlayer instanceof IjkMediaPlayer) {
            ...
            LogUtils.d("只有IjkPlayer才能设置播放速度");
        }
    }
    ...
    public void start() {
        if (mCurrentState == STATE_IDLE) {
            JVPlayerManager.instance().setCurrentJVPlayer(this);
            initAudioManager();
            initMediaPlayer();
            initTextureView();
            addTextureView();
        } else {
            LogUtils.d("NiceVideoPlayer只有在mCurrentState == STATE_IDLE时才能调用start方法.");
        }
    }
    ...
    public void restart() {
        if (mCurrentState == STATE_PAUSED) {
            ...
            LogUtils.d("NiceVideoPlayer在mCurrentState == " + mCurrentState + "时不能调用restart()方法.");
        }
    }
...
//                    ((IjkMediaPlayer)mMediaPlayer).setOption(4, "framedrop", 1L);
    ...
    private void initTextureView() {
        if (mTextureView == null) {
            mTextureView = new JVTextureView(mContext);
    ...
    private void openMediaPlayer() {
        ...
        try {
            ...
            e.printStackTrace();
            LogUtils.e("打开播放器发生错误" + e.toString());
        }
    }
    ...
    private IMediaPlayer.OnPreparedListener mOnPreparedListener
        ...
        public void onPrepared(IMediaPlayer mp) {
            ...
            mController.onPlayStateChanged(mCurrentState);
            LogUtils.d("onPrepared ——> STATE_PREPARED");
            mp.start();
            // 从上次的保存位置播放
            if (continueFromLastPosition) {
                long savedPlayPosition = JVUtil.getSavedPlayPosition(mContext, mUrl);
    ...
    public void enterFullScreen() {
        ...
        // 隐藏ActionBar、状态栏，并横屏
        JVUtil.hideActionBar(mContext);
        JVUtil.scanForActivity(mContext)
                .setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);

        ViewGroup contentView = (ViewGroup) JVUtil.scanForActivity(mContext)
    ...
    public void enterTinyWindow() {
        ...
        FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(
                (int) (JVUtil.getScreenWidth(mContext) * 0.6f),
                (int) (JVUtil.getScreenWidth(mContext) * 0.6f * 9f / 16f));
        params.gravity = Gravity.BOTTOM | Gravity.END;
        params.rightMargin = JVUtil.dp2px(mContext, 8f);
        params.bottomMargin = JVUtil.dp2px(mContext, 8f);
    ...
    public void release() {
        ...
        if (isPlaying() || isBufferingPlaying() || isBufferingPaused() || isPaused()) {
            JVUtil.savePlayPosition(mContext, mUrl, getCurrentPosition());
        } else if (isCompleted()) {
            JVUtil.savePlayPosition(mContext, mUrl, 0);
        }
        // 退出全屏或小窗口
        if (isFullScreen()) {
            exitFullScreen();
