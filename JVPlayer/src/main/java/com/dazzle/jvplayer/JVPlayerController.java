 ...
 * Description:控制器抽象类
 */
public abstract class JVPlayerController
        extends FrameLayout implements View.OnTouchListener {

    private Context mContext;
    protected IJVPlayer mNiceVideoPlayer;
    ...
    public JVPlayerController(Context context) {
        super(context);
        mContext = context;
        this.setOnTouchListener(this);
    }

    public void setNiceVideoPlayer(IJVPlayer niceVideoPlayer) {
     ...
     *                  <ul>
     *                  <li>{@link JVPlayer#STATE_IDLE}</li>
     *                  <li>{@link JVPlayer#STATE_PREPARING}</li>
     *                  <li>{@link JVPlayer#STATE_PREPARED}</li>
     *                  <li>{@link JVPlayer#STATE_PLAYING}</li>
     *                  <li>{@link JVPlayer#STATE_PAUSED}</li>
     *                  <li>{@link JVPlayer#STATE_BUFFERING_PLAYING}</li>
     *                  <li>{@link JVPlayer#STATE_BUFFERING_PAUSED}</li>
     *                  <li>{@link JVPlayer#STATE_ERROR}</li>
     *                  <li>{@link JVPlayer#STATE_COMPLETED}</li>
     *                  </ul>
     */
     ...
     *                 <ul>
     *                 <li>{@link JVPlayer#MODE_NORMAL}</li>
     *                 <li>{@link JVPlayer#MODE_FULL_SCREEN}</li>
     *                 <li>{@link JVPlayer#MODE_TINY_WINDOW}</li>
     *                 </ul>
     */
    ...
    protected void startUpdateProgressTimer() {
        ...
        if (mUpdateProgressTimerTask == null) {
            mUpdateProgressTimerTask = new TimerTask() {
                @Override
                public void run() {
                    JVPlayerController.this.post(new Runnable() {
                        @Override
                        public void run() {
                            updateProgress();
                        }
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (!mNeedChangePosition && !mNeedChangeVolume && !mNeedChangeBrightness) {
                    ...
                    if (absDeltaX >= THRESHOLD) {
                        ...
                        if (mDownX < getWidth() * 0.5f) {
                            ...
                            mGestureDownBrightness = JVUtil.scanForActivity(mContext)
                ...
                if (mNeedChangeBrightness) {
                    ...
                    WindowManager.LayoutParams params = JVUtil.scanForActivity(mContext)
                            .getWindow().getAttributes();
                    params.screenBrightness = newBrightnessPercentage;
                    JVUtil.scanForActivity(mContext).getWindow().setAttributes(params);
